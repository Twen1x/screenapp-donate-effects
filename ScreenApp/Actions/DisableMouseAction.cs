...
/// <summary>
/// Блокировка мыши: ставит низкоуровневый хук WH_MOUSE_LL и «проглатывает» все
/// мышиные события (возврат (IntPtr)1 из процедуры хука прерывает их доставку
...
/// </summary>
public sealed class DisableMouseAction : IScreenAction
{
    ...
    public DisableMouseAction()
    {
        _proc = HookCallback;
    }

    public string ActionId => "disable_mouse";
    ...
    public void Execute()
    {
        ...
        // Для WH_MOUSE_LL hMod может быть NULL (или модуль текущего процесса) и
        ...
        if (_hookHandle == IntPtr.Zero)
        {
            ...
            throw new InvalidOperationException(
                $"Не удалось установить хук мыши (WH_MOUSE_LL). Win32 error {err}. " +
    ...
    public void Revert()
    {
        ...
        {
            int err = Marshal.GetLastWin32Error();
            Debug.WriteLine($"[DisableMouseAction] UnhookWindowsHookEx failed. Win32 error {err}.");
        }
